...
class Counter
  ...
  def self.deliver_tx(trans, _call)
    array_count = 0
    ...
    if @@serial
      if array_count == @@count
        @@count += 1
        @@trans_count += 1
        Types::ResponseDeliverTx.new(code: :OK, data: trans.tx)
      else
  ...
  def self.check_tx(trans, _call)
    ...
    if @@serial
      if array_count == @@count
        Types::ResponseCheckTx.new(code: :OK, data: trans.tx)
      else
  ...
  def self.begin_block(beg, _call)
    beg
  end

  def self.end_block(e, _call)
    e
  end
